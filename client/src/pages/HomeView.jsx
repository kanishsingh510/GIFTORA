...
export default function HomeView({ products = [], apiMode = "connecting" }) {
  ...
  return (
    <div className="animate-fade-in pb-16 sm:pb-24 overflow-x-hidden">
      ...
      {/* ── Hero ─────────────────────────────────────────────────────── */}
      <section className="relative h-[300px] sm:h-[480px] lg:h-[580px] overflow-hidden mb-5 sm:mb-16 bg-slate-900 -mx-4 sm:-mx-6 lg:-mx-8">
        ...
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-center px-5">
          ...
          <span className="inline-block bg-coral/90 text-white text-[10px] sm:text-xs font-black uppercase tracking-widest px-3 py-1 rounded-full mb-3 sm:mb-5">
            ✦ Premium Gifting
          </span>

          <h1 className="text-2xl sm:text-5xl lg:text-6xl font-serif font-bold text-white leading-tight mb-2 sm:mb-4 max-w-[280px] sm:max-w-2xl mx-auto">
            Perfect Gifts for Every Occasion
          </h1>

          <p className="text-xs sm:text-lg font-medium text-white/80 mb-6 sm:mb-8 max-w-[240px] sm:max-w-md mx-auto leading-relaxed">
            Discover unique presents that tell your story
          </p>

          {/* Side-by-side buttons on ALL screen sizes using flex-1 */}
          <div className="flex flex-row gap-2 sm:gap-3 justify-center w-full max-w-[280px] sm:max-w-none sm:w-auto">
            <button
              onClick={() => { navigate("/studio"); window.scrollTo(0, 0); }}
              className="flex-1 sm:flex-none bg-coral text-white font-black text-sm sm:text-base h-10 sm:h-14 px-4 sm:px-10 rounded-xl sm:rounded-2xl hover:bg-[#e24e3c] active:scale-95 transition-all shadow-lg"
            >
              Shop Now
            </button>
            <button
              onClick={() => document.getElementById("shop-categories")?.scrollIntoView({ behavior: "smooth" })}
              className="flex-1 sm:flex-none bg-white/15 backdrop-blur-md text-white border border-white/30 font-black text-sm sm:text-base h-10 sm:h-14 px-4 sm:px-10 rounded-xl sm:rounded-2xl hover:bg-white/25 active:scale-95 transition-all"
            >
              Explore
            </button>
          </div>
      ...
      {/* ── Trust Bar — 4-col grid, no scroll needed ─────────────────── */}
      <section className="max-w-7xl mx-auto mb-8 sm:mb-16">
        <div className="grid grid-cols-4 gap-2 sm:gap-4 p-3 sm:p-6 rounded-2xl sm:rounded-[32px] bg-slate-50 border border-slate-100">
          {[
            ...
            <div key={i} className="flex flex-col items-center gap-1.5 py-1">
              <div className="h-8 w-8 sm:h-10 sm:w-10 shrink-0 rounded-xl bg-white border border-slate-100 shadow-sm flex items-center justify-center text-mint">
                <f.icon size={15} />
              </div>
              <p className="font-black text-[9px] sm:text-xs text-ink leading-tight text-center whitespace-pre-line">
                {f.label}
              </p>
            </div>
          ))}
      ...
      <section id="shop-categories" className="mb-8 sm:mb-24 max-w-7xl mx-auto">
        <div className="text-center mb-5 sm:mb-12">
          <h2 className="text-lg sm:text-4xl font-black text-ink mb-1.5 sm:mb-3">
            Shop by Gift Type
          </h2>
      ...
      <section className="mb-8 sm:mb-24 max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-4 sm:mb-8">
          ...
          <button
            ...
            className="text-[11px] sm:text-xs font-black text-orange-500 uppercase tracking-widest hover:text-ink transition-colors py-2 pl-3"
          >
            View All →
          </button>
        </div>
      ...
      {/* ── Final CTA ────────────────────────────────────────────────── */}
      <section className="mt-6 sm:mt-16 max-w-7xl mx-auto rounded-2xl sm:rounded-[48px] bg-mint p-6 sm:p-16 text-center text-ink relative overflow-hidden">
        <div className="absolute -top-8 -right-8 w-32 h-32 bg-white/20 rounded-full blur-3xl pointer-events-none" />
        <div className="absolute -bottom-8 -left-8 w-32 h-32 bg-ink/10 rounded-full blur-3xl pointer-events-none" />

        <h2 className="relative text-xl sm:text-4xl font-black mb-2 sm:mb-5 leading-snug">
        ...
        <button
          onClick={() => navigate("/studio")}
          className="relative bg-ink text-white font-black h-12 sm:h-14 px-8 sm:px-10 rounded-xl sm:rounded-2xl text-sm sm:text-base hover:bg-slate-800 active:scale-95 transition-all shadow-xl shadow-ink/20 w-full sm:w-auto max-w-[260px]"
        >
          Enter Studio
        </button>
      </section>

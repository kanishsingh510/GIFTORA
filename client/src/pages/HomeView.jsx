...
export default function HomeView({ products = [], apiMode = "connecting" }) {
  ...
  return (
    // ✅ overflow-x-hidden on root prevents any accidental horizontal scroll
    <div className="animate-fade-in pb-24 overflow-x-hidden">

      {/* ─── Hero Section ───────────────────────────────────────────────
          FIX: Use a negative-margin trick WITHOUT width:calc or w-screen.
          -mx-4 pulls the section to the edges; we DON'T add extra width.
          The parent layout already has px-4, so -mx-4 exactly cancels it.
          No inline style, no duplicate `relative`, no translate tricks.
      ──────────────────────────────────────────────────────────────────── */}
      <section className="relative h-[380px] sm:h-[580px] overflow-hidden mb-6 sm:mb-16 bg-slate-900 -mx-4 sm:-mx-6 lg:-mx-8">
        <div className="absolute inset-0 w-full h-full">
          <img
            src="https://images.unsplash.com/photo-1549465220-1a8b9238cd48?auto=format&fit=crop&q=75&w=800"
            srcSet={[
              "https://images.unsplash.com/photo-1549465220-1a8b9238cd48?auto=format&fit=crop&q=65&w=480 480w",
        ...
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-center px-5">
          <div className="max-w-[320px] sm:max-w-4xl mx-auto">
            ...
            <h1 className="text-3xl sm:text-6xl font-serif font-bold mb-3 sm:mb-6 text-white leading-[1.15] sm:leading-tight">
              Perfect Gifts for<br />Every Occasion
            </h1>
            <p className="text-sm sm:text-xl font-medium mb-7 sm:mb-10 text-white/85 drop-shadow-lg max-w-xs sm:max-w-xl mx-auto leading-relaxed">
              Discover unique presents that tell your story
            </p>
            <div className="flex gap-3 sm:gap-4 justify-center">
              <button
                onClick={() => { navigate("/studio"); window.scrollTo(0, 0); }}
                className="bg-coral text-white h-11 sm:h-14 px-6 sm:px-10 rounded-2xl text-sm sm:text-base font-black hover:bg-[#e24e3c] transition-all shadow-xl shadow-coral/30 active:scale-95"
              >
                Shop Now
              </button>
              <button
                onClick={() => document.getElementById("shop-categories")?.scrollIntoView({ behavior: "smooth" })}
                className="bg-white/15 backdrop-blur-md text-white border border-white/30 h-11 sm:h-14 px-6 sm:px-10 rounded-2xl text-sm sm:text-base font-black hover:bg-white/25 transition-all active:scale-95"
              >
                Explore
              </button>
            </div>
      ...
      {/* ─── Categories ──────────────────────────────────────────────── */}
      <section id="shop-categories" className="mb-10 sm:mb-28 max-w-7xl mx-auto">
        <div className="text-center mb-6 sm:mb-14">
          <h2 className="text-xl sm:text-4xl font-black text-ink mb-2 sm:mb-3">Shop by Gift Type</h2>
                ...
                products.reduce((acc, p) => {
                  ...
                  if (!acc[cat]) {
                    acc[cat] = {
                      category: cat,
                      count: 0,
                      // ✅ optimise any product image on the fly
      ...
      {/* ─── Bestsellers ─────────────────────────────────────────────── */}
      <section className="mb-10 sm:mb-28 max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-5 sm:mb-10">
          ...
          <button
            ...
            className="text-xs font-black text-orange-500 uppercase tracking-widest hover:text-ink transition-colors"
          >
            View All →
          </button>
        </div>
      ...
      {/* ─── Trust Bar ───────────────────────────────────────────────── */}
      <section className="max-w-7xl mx-auto mb-10 sm:mb-28">
      ...
      <section className="mt-8 sm:mt-20 max-w-7xl mx-auto rounded-3xl sm:rounded-[60px] bg-mint p-8 sm:p-16 text-center text-ink relative overflow-hidden">
        ...
        <div className="absolute -bottom-10 -left-10 w-40 h-40 bg-ink/10 rounded-full blur-3xl pointer-events-none" />

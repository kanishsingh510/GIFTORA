...
export default function HomeView({ products = [], apiMode = "connecting" }) {
  ...
  if (isMobile) {
    return (
      <div className="animate-fade-in flex flex-col gap-8 pb-32 pt-2 px-0.5 overflow-x-hidden">
        ...
        {/* Stories */}
        <div className="flex overflow-x-auto scrollbar-none gap-5 px-1 pb-1">
          {categories.map((cat) => (
            <div key={cat.id} onClick={() => navigate(`/studio?category=${cat.id}`)} className="flex flex-col items-center gap-1.5 shrink-0">
              <div className="w-16 h-16 rounded-full p-0.5 border-2 border-coral shadow-sm overflow-hidden">
        ...
        {/* Hero Card - Totally standard width */}
        <div 
          onClick={() => navigate("/studio")}
          className="relative h-[400px] rounded-[32px] overflow-hidden bg-slate-900 border border-slate-100/10 shadow-lg mx-1"
        >
          <img 
          ...
          <div className="absolute bottom-0 inset-x-0 p-8">
            <span className="inline-block bg-white/20 backdrop-blur-md text-white text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full mb-4 border border-white/30">
              ✦ Featured Gift
            </span>
            <h2 className="text-3xl font-serif font-bold text-white mb-2 leading-tight">Your perfect<br />gift awaits.</h2>
            <p className="text-white/80 text-xs font-medium mb-6 leading-relaxed max-w-[200px]">Design a memory in our real-time studio.</p>
            <button className="w-full bg-coral text-white h-12 rounded-2xl font-black text-sm shadow-xl shadow-coral/20">
              Start Creating
            </button>
          </div>
        </div>

        {/* Trust Bar - No marquee for stability */}
        <div className="grid grid-cols-2 gap-3 mx-1">
          {[
            { icon: Gift, label: "Premium Packing" },
            { icon: Truck, label: "Fast Delivery" },
            { icon: ShieldCheck, label: "Best Quality" },
            { icon: ShoppingBag, label: "Bulk Gifts" },
          ].map((f, i) => (
            <div key={i} className="flex items-center gap-3 p-3 bg-slate-50 border border-slate-100 rounded-2xl">
              <f.icon size={16} className="text-mint" />
              <span className="text-[10px] font-black uppercase tracking-tight text-ink">{f.label}</span>
            </div>
          ))}
        </div>
  ...
  return (
    <div className="animate-fade-in pb-24 overflow-hidden">
      ...
      <section id="shop-categories" className="mb-24 max-w-7xl mx-auto px-4">
